...
import ntk.android.base.adapter.SingleChooseAdapter;
import ntk.android.base.utill.FontManager;

public class RadioListDialog extends DialogFragment {
    SingleChooseAdapter adapter;
    BiFunction<String, Integer, Void> function;

    public static RadioListDialog newInstance(BiFunction<String, Integer, Void> func, List<String> list) {
        RadioListDialog dialog = new RadioListDialog();
        dialog.setList(list);
        dialog.setInterface(func);
        return dialog;
    }

    private void setInterface(BiFunction<String, Integer, Void> o) {
        function = o;
    }
        ...
        button.setOnClickListener(view1 -> {
            if (adapter != null) {
                if (adapter.getIndex() != null) {
                    function.apply(adapter.getIndex(), adapter.list().indexOf(adapter.getIndex()));
                    dismiss();
                }
            }
        });
    ...
    public void setList(List<String> list) {
        adapter = new SingleChooseAdapter(list);
    }
